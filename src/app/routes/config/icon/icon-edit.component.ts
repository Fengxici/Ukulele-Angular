...
export class IconEditComponent {
  ...
  schema: SFSchema = {
    properties: {
      type: { type: 'string', title: '类型',enum: [
        { label: 'class', value: 'class' },
        { label: 'icon', value: 'icon' },
        { label: 'img', value: 'img' }
      ] },
      value: { type: 'string', title: '值', maxLength: 15 },
      theme: { type: 'string', title: '主题' , enum: [
  ...
  save(value: any) {
    console.log(value);
    if(this.record.id){
      this.http.put('api/portal-service/ant-icon',value).subscribe((res:any)=>{
        if(res){
      ...
      this.http.post('api/portal-service/ant-icon',value).subscribe((res:any)=>{
        if(res){
          if(res.code===ResponseCode.SUCCESS){
